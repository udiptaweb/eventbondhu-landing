...
<section class="py-10 sm:py-14" style="background:{{ $t['body_bg'] }}">
    <div class="max-w-3xl mx-auto px-4 sm:px-6">
        <div class="text-center mb-8 reveal">
            ...
            <p class="text-sm" style="color:{{ $t['dark'] ? $t['card_muted'] : '#6b7280' }}">
                Reach out directly or book securely via the Event Bondhu app
            </p>
        </div>

        <div class="space-y-3 mb-6 reveal delay-100">
            {{-- WhatsApp (most prominent) --}}
            @if($whatsapp_url)
            <a href="{{ $whatsapp_url }}" target="_blank" rel="nofollow noopener"
               class="flex items-center justify-center gap-3 w-full py-4 rounded-2xl font-semibold text-base transition-all hover:opacity-90 hover:scale-[1.02] shadow-lg"
            ...
            <a href="tel:{{ preg_replace('/[^0-9+]/', '', $phone) }}"
               class="flex items-center justify-center gap-3 w-full py-3.5 rounded-2xl font-semibold text-sm transition-all hover:opacity-90"
               style="background:{{ $t['card_bg'] }}; color:{{ $t['accent'] }}; border:1px solid {{ $t['badge_bdr'] }}">
        ...
        {{-- App CTA divider --}}
        <div class="relative my-6 reveal delay-200">
            <div class="absolute inset-y-0 left-0 right-0 flex items-center">
                <div class="w-full border-t" style="border-color:{{ $t['card_bdr'] }}"></div>
            </div>
            <div class="relative flex justify-center">
                <span class="px-4 text-xs font-medium" style="background:{{ $t['body_bg'] }}; color:{{ $t['dark'] ? $t['card_muted'] : '#9ca3af' }}">
                    or book securely on the app
                </span>
            </div>
        </div>

        <div class="rounded-2xl p-6 text-center reveal delay-300"
             style="background:{{ $t['card_bg'] }}; border:1px solid {{ $t['card_bdr'] }}">
